...
static void capture_cam_thread(void * arg)
{
    ...
    u16 * buffer = (u16 *)linearAlloc(bufsz);
...
{
    if((data->cam_thread = threadCreate(capture_cam_thread, data, 0x10000, 0x1A, 0, false)) == NULL)
    {
        printf("cam thread is null\n");
        //throw_error(language.camera.thread_error, ERROR_LEVEL_ERROR);
        LightEvent_Signal(&data->event_cam_info);
        LightEvent_Signal(&data->event_ui_info);
        return false;
    }
    if((data->ui_thread = threadCreate(update_ui, data, 0x10000, 0x1A, 0, false)) == NULL)
    {
        printf("ui thread is null\n");
        LightEvent_Signal(&data->event_ui_info);
        return false;
    }
    return true;
...
char *init_qr()
{
    ...
    start_qr(&data);
    quirc_data scan_data;
    const bool ready = start_capture_cam(&data);
    bool finished = !ready;
    if (ready) {
        printf("QR scanner ready! Scan a QR code! Press R/B/Touch to cancel\n");
    } else {
        printf("QR scanner error!\n");
    }
    while(!finished)
    {
        hidScanInput();
    ...
    if(finished && ready)
    {
        ...
        char *qrData = strdup((char *)(scan_data.payload));
